    ...
    <>
      ...
      <div className="new-entry-container">
        {!showForm ? ( // Show carousel if the form is not yet shown
          <>
            <h2 className="new-entry-header">What is this entry about?</h2>

            {/* Custom carousel for category selection */}
            <div className="carousel-container">
              <button className="prevBtn" onClick={goToNext}>&#10094;</button>
              <div className="carousel-slide">
                {categories.map((category, index) => {
                  // Define card class based on the current carousel index
                  let cardClass = "category-card";
                  if (index === currentIndex) cardClass += " center-card";
                  else if (index === (currentIndex - 1 + categories.length) % categories.length) cardClass += " left-card";
                  else if (index === (currentIndex + 1) % categories.length) cardClass += " right-card";
                  else cardClass += " back-card";

                  return (
                    <div key={category.label} className={cardClass} onClick={() => handleCategorySelect(category.label)}>
                      {category.imgSrc && <img src={category.imgSrc} alt={category.label} />}
              ...
              <button className="nextBtn" onClick={goToPrev}>&#10095;</button>
            </div>
          </>
        ) : ( // Show form once a category is selected
          <>
            <h2 className="new-entry-header">New {selectedCategory} Entry</h2>
            ...
            <div className="button-container">
              ...
              <button className="button" onClick={() => setShowForm(false)}>Back</button> {/* Button to go back to the carousel */}
            </div>
          </>
        )}
      </div>
